...
def interpret_indicators(current, history, coin_name, my_coin_name):
    ...
    if current < avg_price:
        trend_desc = f"📉 {coin_name} **في انخفاض** مقابل {my_coin_name}."
    elif current > avg_price:
        trend_desc = f"📈 {coin_name} **في ارتفاع** مقابل {my_coin_name}."
    else:
        trend_desc = f"➖ {coin_name} **سعره ثابت** مقابل {my_coin_name}."
...
def analyze_conversion(amount, from_curr, to_curr, market_data):
    ...
    target_history_for_analysis = []
    
    if to_curr == "ILS":
        analyze_val = rate_from
        analyze_hist = market_data[from_curr]["history"]
        coin_label = market_data[from_curr]["name"] 
        base_label = market_data[to_curr]["name"]   
    
    elif from_curr == "ILS":
        analyze_val = rate_to
        analyze_hist = market_data[to_curr]["history"]
        coin_label = market_data[to_curr]["name"]
        base_label = market_data[from_curr]["name"]

    else:
        analyze_val = exchange_rate
        hist_from = market_data[from_curr]["history"]
        hist_to = market_data[to_curr]["history"]
    ...
    text += f"   {trend_msg}\n"
    text += f"   📊 **وضع السوق:** {rsi_msg}\n"
    
    if not ({from_curr, to_curr} == {"USD", "JOD"}):
        avg_hist = mean(analyze_hist)
        if analyze_val < avg_hist:
             text += f"\n💡 **فرصة:** السعر الحالي ({analyze_val:.2f}) أقل من المعدل، فرصة جيدة.\n"
...
def start(message):
    bot.reply_to(message, "أهلاً بك 🇵🇸\nاكتب المبلغ والعملة (مثلاً: **100 دولار**)")

@bot.message_handler(func=lambda m: True)
def handle_message(message):
    text = message.text.lower()
    # إزالة رسالة الانتظار لتسريع الاستجابة محلياً
    ...
    try:
        ...
        if not amount_str:
            bot.reply_to(message, "اكتب رقم واضح (مثال: 200 دينار)")
            return
        ...
        report += "ــــــــــــــــــــــــــــــــ\n"
        targets = [c for c in ["USD", "JOD", "ILS"] if c != curr_code]
        for target in targets:
            report += analyze_conversion(amount, curr_code, target, market_data)

        bot.reply_to(message, report)

    except Exception as e:
        bot.reply_to(message, "حدث خطأ، تأكد من الرقم.")

print("Bot V9 (Local) is running...")
